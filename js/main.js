...
mainInfo.innerHTML = '<div id="sg-shoppingGuide"><ul class="sg-nav sg-nav-tabs"><li class="sg-active">卖家服务情况</li><li>如何识别网购真假</li></ul><div><div class="sg-nav-content"><ul><li><p>开店时长: {{shopStartDate[0]}}年{{shopStartDate[1]}}个月</p></li><li><p>近30天退款率为 {{ratRefund.localVal}}%, 退款 {{ratRefund.refundCount}} 次<span class="sg-serious-warn {{if !ratRefund.danger}}hide{{/if}}">高于行业平均值</span></p><p>近30天纠纷率为 {{complaints.localVal}}%, 纠纷退款 {{complaints.disputRefundNum}} 笔<span class="sg-serious-warn {{if !complaints.danger}}hide{{/if}}">高于行业平均值</span></p><p>近30天被处罚 {{punish.punishCount}} 次, 出售假冒商品 {{punish.cPunishTimes}} 次, 虚假交易 {{punish.xujiaTimes}} 次<span class="sg-serious-warn {{if !punish.danger}}hide{{/if}}">高于行业平均值</span></p></li><li><p class="sg-note {{if +(anonyCommentsRate)>95}}sg-warn{{/if}}">评价匿名率:<span class="sg-rate">{{anonyCommentsRate}}%</span><span class="tb-r-sku">卖家可能在刷信用</span></p><p class="sg-note {{if +(anonyTradeRate)>95}}sg-warn{{/if}}">成交记录匿名率:<span class="sg-rate">{{anonyTradeRate}}%</span><span class="tb-r-sku">卖家可能在刷信用</span></p><p class="sg-note {{if +(successRate)<30}}sg-warn{{/if}}">交易成功比例:<span class="sg-rate">{{successRate}}%</span><span class="tb-r-sku">卖家可能在刷交易记录</span></p></li><li><p class="sg-evaluate"><span>商品中差评:<span id="sg-showBad" class="sg-link {{if !(badCommentList&&badCommentList.length)}}tb-r-sku{{/if}}">{{(badCommentList&&badCommentList.length) || 0}} 条</span></span><span>卖家所有中差评:<span id="sg-showAllBad" class="sg-link {{if !(allBadCommentList&&allBadCommentList.length)}}tb-r-sku{{/if}}">{{(allBadCommentList&&allBadCommentList.length)|| 0}} 条</span></span><span>查看所有</span></p></li></ul></div><div class="sg-nav-content hide"><ul><li>哈哈, 你能看到我吗</li></ul></div></div></div>';
